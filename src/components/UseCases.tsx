...
const UseCases: React.FC = () => {
  ...
  return (
    <section className="relative h-screen w-full overflow-hidden">
      ...
      <div className="relative h-full flex items-center">
        <div className="max-w-7xl mx-auto px-6 w-full">
          <div className="max-w-2xl">
            <p className="text-6xl font-light text-white/40 mb-4">
      ...
      <button
        onClick={prevSlide}
        className="absolute left-8 top-1/2 -translate-y-1/2 w-12 h-12 rounded-full border-2 border-white/50 flex items-center justify-center text-white hover:bg-white/10 transition-all"
      >
        <ChevronLeft size={24} />
      </button>
      <button
        onClick={nextSlide}
        className="absolute right-8 top-1/2 -translate-y-1/2 w-12 h-12 rounded-full border-2 border-white/50 flex items-center justify-center text-white hover:bg-white/10 transition-all"
      >
          ...
          <div className="flex overflow-x-auto space-x-4 scrollbar-hide">
            {useCases.map((useCase, index) => (
              <button
                ...
                className={`flex-shrink-0 text-left px-4 py-2 transition-all ${
                  currentSlide === index
